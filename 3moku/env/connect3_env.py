...
class Connect3Env:
    ...
    def _check_win(self, player):
        ...
        for r in range(self.ROWS):
            for c in range(self.COLS - wl + 1):
                if all(b[r][c + i] == player for i in range(wl)):
                    return True
        # 縦
        for r in range(self.ROWS - wl + 1):
            for c in range(self.COLS):
                if all(b[r + i][c] == player for i in range(wl)):
                    return True
        # 斜め右下
        for r in range(self.ROWS - wl + 1):
            for c in range(self.COLS - wl + 1):
                if all(b[r + i][c + i] == player for i in range(wl)):
                    return True
        # 斜め左下
        for r in range(self.ROWS - wl + 1):
            for c in range(wl - 1, self.COLS):
                if all(b[r + i][c - i] == player for i in range(wl)):
                    return True
        return False
