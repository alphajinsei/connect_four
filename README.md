...
- **現在の方針**: 先手（PLAYER1）固定、カリキュラム学習（noise段階的低下 → 完全RuleBased）+ 極小中間報酬 + 遅延コールバック方式
...
| **9** | **カリキュラム + 極小中間報酬（3連+0.03, 防御+0.02, 見逃し-0.05）** | **実行中** |
...
```
自分の3連を作った         → +0.03  ← 攻撃の動機
相手の3連を防いだ         → +0.02  ← 防御の動機
相手の勝ち手を見逃した    → -0.05  ← 防御の失敗に気づかせる
```

**勝敗報酬（±1.0）が圧倒的に大きいことが最重要**です。中間報酬の1ゲーム累積は±0.06〜0.09（勝敗の約1/10以下）に抑えています。過去のステージでは中間報酬が大きすぎて学習を歪めた（ステージ5: 累積200%超、ステージ7: 累積20〜25%）一方、中間報酬ゼロでは防御を学べなかった（ステージ8: Phase 2で30000ep停滞）。
...
> **教訓:** 中間報酬が「多すぎる」のも「ゼロ」も問題。1ゲーム累積が勝敗報酬の1/10以下になる極小スケールが適切。

---

## 現行方針の設計思想（ステージ9）

**ステージ8の失敗（中間報酬ゼロ→防御学習不能）を受けて、極小中間報酬を再導入:**

### 変更点（ステージ8 → 9）

1. **極小中間報酬を再導入**: 3連作成+0.03、3連防御+0.02、見逃し-0.05
2. **2連報酬を廃止**: ノイズが多すぎて累積を増やすだけ
...
- **極小中間報酬**: 1ゲーム累積 ≈ ±0.06〜0.09（勝敗の約1/10以下）
- **チェックポイント保存**: ReplayBuffer + ε + Adam状態を `_checkpoint.pt` に保存
- **スナップショットは2系統**: `best_ep*` + `phaseup_ph*`
- **eval負荷削減**: eval_interval=1000、vs Noisy 200戦、vs RuleBased 100戦（学習スループット2〜3倍改善）
