...
| 9 | カリキュラム + 極小中間報酬（MLP） | 廃止（Phase 1で50000ep停滞、ステージ8より悪化） |
| **10** | **CNN + カリキュラム + 中間報酬なし** | **実行中** |
...
極小中間報酬（3連+0.03, 防御+0.02, 見逃し-0.05）を再導入。Phase 1（noise=0.8）で**50000エピソード学習しても突破できず**。vs Noisy 69.5%（ベスト）→20%台に崩壊→74%に回復→45%に再停滞、と激しく振動。ステージ8（Phase 1突破済み）より明確に悪化。極小スケールでもQ値の安定性を壊した。
