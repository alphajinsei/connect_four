...
**ステージ7から中間報酬を完全廃止し、勝敗報酬（±1.0）のみでPhase 1から再学習:**
...
- **中間報酬なし**: 勝敗報酬（±1.0）のみ。中間報酬なしでのベースラインを取得し、必要なら後から最小限の再導入を検討
- **チェックポイント保存**: ReplayBuffer + ε + Adam状態を `_checkpoint.npz` に保存
