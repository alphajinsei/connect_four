...
| **7** | **シンプルカリキュラム（noise段階的低下 → 完全RuleBased）** | **実行中** |
...
**ステージ6を簡素化・改善してPhase 1から再学習:**

### 変更点（ステージ6 → 7）

1. **Phase 1 目標 80% → 75%**: noise=0.8での足踏みを解消
2. **Phase 4 スナップショットプール廃止**: 純粋に noise=0.1 のみ（プール方式はステージ5で失敗確認済み）
3. **Phase 5 追加**: noise=0.0（完全ルールベースAI）、目標40%
